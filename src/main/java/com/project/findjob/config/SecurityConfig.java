...
import com.project.findjob.jwt.JwtFilter;
import com.project.findjob.jwt.LoginFilter;
   ...
   private final JWTUtil jwtUtil;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .formLogin((auth)-> auth.disable()) // jwt를 사용하기위해 formLogin 비활성화, 로그인필터 커스텀으로 구현
                .httpBasic((auth)-> auth.disable())
                .authorizeHttpRequests((requests) -> requests
                        .requestMatchers("/", "/css/**","/image/**","/regist/**","/js/**","/login","/main").permitAll() // 다음 url은 인증없이 모든 사용자에게 허용
